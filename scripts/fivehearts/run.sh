#!/bin/bash
# Runs the aarch64 unityloader against FiveHearts inside the bogo-arm64-test
# container (see Dockerfile.test). The host mounts the game root at /game
# (gamedata/ conf/ cache/) plus this repo's loader and container toml at /boot.
#
#   SECS=40 bash /work/scripts/fivehearts/run.sh
#
...
cd /game || exit 1
# The binary has to live on the container's own overlay: ld.so mmaps the ELF
# straight off the bind mount and a short read comes back as
...
# "Could not change directory to "). game_files stays "./gamedata/" so the
# process still needs /game as its cwd - only these two files are copied out.
